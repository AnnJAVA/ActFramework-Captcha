package cn.anline.ann.controller;
...
import cn.anline.ann.model.UserModel;
...
public class Home {
    ...
    @Inject
    private EbeanDao<Integer,UserModel> userModelEbeanDao;

    @GetAction("")
    public void index(){
        Controller.Util.render("index.html");
    }

    @GetAction("login")
    public void login(){
        Controller.Util.render("/login");
    }
    ...
    public Object captcha() throws IOException {
        ...
        //7.将随机数存在session中
        session.cache("checkcode",random);
        context.resp().contentType("image/jpeg");//等同于response.setHeader("Content-Type", "image/jpeg");
        //9.设置响应头控制浏览器不要缓存
        context.resp().addHeader("expries", String.valueOf(-1));
        context.resp().addHeader("Cache-Control", "no-cache");
        context.resp().addHeader("Pragma", "no-cache");
        //10.将图片写给浏览器
        return ImageIO.write(bi, "jpg",context.resp().outputStream());
    }
    @PostAction("login")
    public String doLogin(String username,String password,String captcha){
        String v2 = session.cached("checkcode");

        System.out.println("登录结果得到的验证码");
        System.out.println(v2);
        return "您输入的验证码："+ captcha+"  生成的验证码：" + v2;
    }
